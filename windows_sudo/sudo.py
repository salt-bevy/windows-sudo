...
if __name__ == "__main__":
    if len(sys.argv) == 1 or sys.argv[1] in ["--help", "-h", "su", "/?", "/help"]:
        ...
        if not whole_machine or isUserAdmin():
            ...
            print()
            print('NOTE: a brand-new window is not enough by itself. Windows only hands out the')
            print('updated PATH/PATHEXT to processes started fresh from Explorer (Start menu,')
            print('Win+R, a new File Explorer window, etc). A new tab or window opened inside an')
            print('already-running terminal host (Windows Terminal, an IDE\'s integrated')
            print('terminal, ...) inherits that host\'s OLD environment and will NOT see the')
            print('change -- close every window of that host application (not just one tab) and')
            print('reopen it, or log off and back on, before testing "sudo".')
            time.sleep(5)
        else:
